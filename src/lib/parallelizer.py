...
class Parallelizer(object):
    """
    Parallizer
    This takes a keras model and parallelizes it between multiple GPUs.
    gpu_list
    ...
    def transform(self, model):
        ...
        def get_slice(data, idx, parts):
            ...
            stride = K.concatenate(
                [minibatch_size // parts, features * 0], axis=0)
            if is_last_slice:
                # feed everything else if it's the last slice
                size = K.concatenate([[-1], features], axis=0)
            else:
                size = K.concatenate(
                    [minibatch_size // parts, features], axis=0)
            begin = stride * idx
